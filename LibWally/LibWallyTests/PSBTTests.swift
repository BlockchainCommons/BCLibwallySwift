...
class PSBTTests: XCTestCase {
    ...
    // Paths
    let path0 = try! BIP32Path(string: "m/0'/0'/0'")
    let path1 = try! BIP32Path(string: "m/0'/0'/1'")
    let path2 = try! BIP32Path(string: "m/0'/0'/2'")
    let path3 = try! BIP32Path(string: "m/0'/0'/3'")
    let path4 = try! BIP32Path(string: "m/0'/0'/4'")
    let path5 = try! BIP32Path(string: "m/0'/0'/5'")
    ...
    // Public keys
    let pubKey0 = try! PubKey(Data(hex: "029583bf39ae0a609747ad199addd634fa6108559d6c5cd39b4c2183f1ab96e07f"), network: .testnet)
    let pubKey1 = try! PubKey(Data(hex: "02dab61ff49a14db6a7d02b0cd1fbb78fc4b18312b5b4e54dae4dba2fbfef536d7"), network: .testnet)
    let pubKey2 = try! PubKey(Data(hex: "03089dc10c7ac6db54f91329af617333db388cead0c231f723379d1b99030b02dc"), network: .testnet)
    let pubKey3 = try! PubKey(Data(hex: "023add904f3d6dcf59ddb906b0dee23529b7ffb9ed50e5e86151926860221f0e73"), network: .testnet)
    let pubKey4 = try! PubKey(Data(hex: "03a9a4c37f5996d3aa25dbac6b570af0650394492942460b354753ed9eeca58771"), network: .testnet)
    let pubKey5 = try! PubKey(Data(hex: "027f6399757d2eff55a136ad02c684b1838b6556e5f1b6b34282a94b6b50051096"), network: .testnet)
    ...
    func testSignWithKey() throws {
        ...
        let p213 = try psbt2.signed(with: privKey1).signed(with: privKey3)
