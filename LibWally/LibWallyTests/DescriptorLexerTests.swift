...
class DescriptorLexerTests: XCTestCase {
    func testLexDelimiters() {
        XCTAssertEqual(DescriptorLexer.debugLex(string: "()[]{},"), "(openParen 0..<1), (closeParen 1..<2), (openBracket 2..<3), (closeBracket 3..<4), (openBrace 4..<5), (closeBrace 5..<6), (comma 6..<7)")
    }
    
    func testLexKeywords() {
        XCTAssertEqual(DescriptorLexer.debugLex(string: "sh,wsh,pk,pkh,wpkh,combo,multi,sortedmulti,tr,addr,raw"), "(sh 0..<2), (comma 2..<3), (wsh 3..<6), (comma 6..<7), (pk 7..<9), (comma 9..<10), (pkh 10..<13), (comma 13..<14), (wpkh 14..<18), (comma 18..<19), (combo 19..<24), (comma 24..<25), (multi 25..<30), (comma 30..<31), (sortedmulti 31..<42), (comma 42..<43), (tr 43..<45), (comma 45..<46), (addr 46..<50), (comma 50..<51), (raw 51..<54)")
    }
    
    func testLexAddress() {
        XCTAssertEqual(DescriptorLexer.debugLex(string: "1BvBMSEYstWetqTFn5Au4m4GFg7xJaNVN2"), "(address 0..<34)")
        XCTAssertEqual(DescriptorLexer.debugLex(string: "3J98t1WpEZ73CNmQviecrnyiWrnqRhWNLy"), "(address 0..<34)")
        XCTAssertEqual(DescriptorLexer.debugLex(string: "bc1qar0srrr7xfkvy5l643lydnw9re59gtzzwf5mdq"), "(address 0..<42)")
    }
    
    func testLexData() {
        XCTAssertEqual(DescriptorLexer.debugLex(string: "00112233445566778899aabbccddeeff"), "(data 0..<32)")
    }
    ...
    func testLexDescriptors() {
        XCTAssertEqual(DescriptorLexer.debugLex(string: desc1), "(pk 0..<2), (openParen 2..<3), (data 3..<69), (closeParen 69..<70)")
        XCTAssertEqual(DescriptorLexer.debugLex(string: desc2), "(pkh 0..<3), (openParen 3..<4), (data 4..<70), (closeParen 70..<71)")
        XCTAssertEqual(DescriptorLexer.debugLex(string: desc3), "(wpkh 0..<4), (openParen 4..<5), (data 5..<71), (closeParen 71..<72)")
        XCTAssertEqual(DescriptorLexer.debugLex(string: desc4), "(sh 0..<2), (openParen 2..<3), (wpkh 3..<7), (openParen 7..<8), (data 8..<74), (closeParen 74..<75), (closeParen 75..<76)")
        XCTAssertEqual(DescriptorLexer.debugLex(string: desc5), "(combo 0..<5), (openParen 5..<6), (data 6..<72), (closeParen 72..<73)")
        XCTAssertEqual(DescriptorLexer.debugLex(string: desc6), "(sh 0..<2), (openParen 2..<3), (wsh 3..<6), (openParen 6..<7), (pkh 7..<10), (openParen 10..<11), (data 11..<77), (closeParen 77..<78), (closeParen 78..<79), (closeParen 79..<80)")
        XCTAssertEqual(DescriptorLexer.debugLex(string: desc7), "(multi 0..<5), (openParen 5..<6), (int 6..<7), (comma 7..<8), (data 8..<74), (comma 74..<75), (data 75..<141), (closeParen 141..<142)")
        XCTAssertEqual(DescriptorLexer.debugLex(string: desc8), "(sh 0..<2), (openParen 2..<3), (multi 3..<8), (openParen 8..<9), (int 9..<10), (comma 10..<11), (data 11..<77), (comma 77..<78), (data 78..<144), (closeParen 144..<145), (closeParen 145..<146)")
        XCTAssertEqual(DescriptorLexer.debugLex(string: desc9), "(sh 0..<2), (openParen 2..<3), (sortedmulti 3..<14), (openParen 14..<15), (int 15..<16), (comma 16..<17), (data 17..<83), (comma 83..<84), (data 84..<150), (closeParen 150..<151), (closeParen 151..<152)")
        XCTAssertEqual(DescriptorLexer.debugLex(string: desc10), "(wsh 0..<3), (openParen 3..<4), (multi 4..<9), (openParen 9..<10), (int 10..<11), (comma 11..<12), (data 12..<78), (comma 78..<79), (data 79..<145), (comma 145..<146), (data 146..<212), (closeParen 212..<213), (closeParen 213..<214)")
        XCTAssertEqual(DescriptorLexer.debugLex(string: desc11), "(sh 0..<2), (openParen 2..<3), (wsh 3..<6), (openParen 6..<7), (multi 7..<12), (openParen 12..<13), (int 13..<14), (comma 14..<15), (data 15..<81), (comma 81..<82), (data 82..<148), (comma 148..<149), (data 149..<215), (closeParen 215..<216), (closeParen 216..<217), (closeParen 217..<218)")
        XCTAssertEqual(DescriptorLexer.debugLex(string: desc12), "(pk 0..<2), (openParen 2..<3), (hdKey 3..<114), (closeParen 114..<115)")
        XCTAssertEqual(DescriptorLexer.debugLex(string: desc13), "(pkh 0..<3), (openParen 3..<4), (hdKey 4..<115), (slash 115..<116), (int 116..<117), (slash 117..<118), (int 118..<119), (closeParen 119..<120)")
        XCTAssertEqual(DescriptorLexer.debugLex(string: desc14), "(pkh 0..<3), (openParen 3..<4), (openBracket 4..<5), (data 5..<13), (slash 13..<14), (int 14..<16), (isHardened 16..<17), (slash 17..<18), (int 18..<19), (isHardened 19..<20), (slash 20..<21), (int 21..<22), (isHardened 22..<23), (closeBracket 23..<24), (hdKey 24..<135), (slash 135..<136), (int 136..<137), (slash 137..<138), (star 138..<139), (closeParen 139..<140)")
        XCTAssertEqual(DescriptorLexer.debugLex(string: desc15), "(wsh 0..<3), (openParen 3..<4), (multi 4..<9), (openParen 9..<10), (int 10..<11), (comma 11..<12), (hdKey 12..<123), (slash 123..<124), (int 124..<125), (slash 125..<126), (int 126..<127), (slash 127..<128), (star 128..<129), (comma 129..<130), (hdKey 130..<241), (slash 241..<242), (int 242..<243), (slash 243..<244), (int 244..<245), (slash 245..<246), (star 246..<247), (closeParen 247..<248), (closeParen 248..<249)")
        XCTAssertEqual(DescriptorLexer.debugLex(string: desc16), "(wsh 0..<3), (openParen 3..<4), (sortedmulti 4..<15), (openParen 15..<16), (int 16..<17), (comma 17..<18), (hdKey 18..<129), (slash 129..<130), (int 130..<131), (slash 131..<132), (int 132..<133), (slash 133..<134), (star 134..<135), (comma 135..<136), (hdKey 136..<247), (slash 247..<248), (int 248..<249), (slash 249..<250), (int 250..<251), (slash 251..<252), (star 252..<253), (closeParen 253..<254), (closeParen 254..<255)")
        XCTAssertEqual(DescriptorLexer.debugLex(string: desc17), "(tr 0..<2), (openParen 2..<3), (data 3..<67), (comma 67..<68), (openBrace 68..<69), (pk 69..<71), (openParen 71..<72), (data 72..<136), (closeParen 136..<137), (comma 137..<138), (pk 138..<140), (openParen 140..<141), (data 141..<205), (closeParen 205..<206), (closeBrace 206..<207), (closeParen 207..<208)")
    }
}
